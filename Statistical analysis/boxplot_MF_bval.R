...
data_long <- pivot_longer(data, cols = c(b1000, b3000, b5000, all_bvals), names_to = "condition", values_to = "value")
...
condition_order <- c("b1000", "b3000", "b5000", "all shells")
...
p <- ggplot(data_long, aes(x = condition, y = value, fill = condition)) +
  ...
  labs(x = "Shell used",
       y = "frac_csf") +
  ylim(0.0, 0.2) +  # Sets the limits of the y-axis
  theme_minimal()+
